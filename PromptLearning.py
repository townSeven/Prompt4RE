...
def run_task(task="crime", text_embs=None, region_embs=None, device="cuda",
    ...
    """运行不同任务并保存结果"""
    if task == "checkIn":
        labels = np.load(f"./data/check_counts.npy")
        mask = labels > 0
        labels, text_embs, region_embs = labels[mask], text_embs[mask], region_embs[mask]
    elif task == "crime":
        labels = np.load(f"./data/crime_counts.npy")
    elif task == "serviceCall":
        labels = np.load(f"./data/serviceCall_counts.npy")
    else:
...
if __name__ == '__main__':
    ...
    if task == "crime":
        region_emb_path = f"best_emb_crime.npy"
        prompt_emb_path = f"./data/prompt_embs_{args.model}_crime.npy"
        model_params = {
            "epochs": 3000,
            "lr": 0.0001,
            "text_output_dim": 144
        ...
        region_emb_path = f"best_emb_checkIn.npy"
        prompt_emb_path = f"./data/prompt_embs_{args.model}_checkIn.npy"
        model_params = {
            "epochs": 3000,
            "lr": 0.0003,
            "text_output_dim": 144
        ...
        region_emb_path = f"best_emb_serviceCall.npy"
        prompt_emb_path = f"./data/prompt_embs_{args.model}_serviceCall.npy"
        model_params = {
            "epochs": 3000,
            "lr": 0.0001,
            "text_output_dim": 144
